...
use crate::context::{StorageAccess, StorageId, SubStorageAccess};
...
impl Inner {
    pub fn new(init_chain: Box<dyn InitChain>) -> Self {
        Self::default()
    }

    fn set_storage(&mut self, storage: &Arc<Mutex<dyn StorageAccess>>) {
        self.current_storage = Arc::clone(storage);
        for (id, (ref name, ref mut stateful)) in self.stateful.iter_mut().enumerate() {
...
impl StorageAccess for NoOpStorage {
    fn get(&self, storage_id: u16, key: &dyn AsRef<[u8]>) -> Option<Vec<u8>> {
        Some(Vec::default())
    }

    fn set(&mut self, storage_id: u16, key: &dyn AsRef<[u8]>, value: Vec<u8>) {}

    fn has(&self, storage_id: u16, key: &dyn AsRef<[u8]>) -> bool {
        false
    }

    fn remove(&mut self, storage_id: u16, key: &dyn AsRef<[u8]>) {}
...
impl Initializer for Coordinator {
    fn initialize_chain(&self, storage: Arc<Mutex<dyn StorageAccess>>) -> (CompactValidatorSet, ConsensusParams) {
        ...
        inner.max_body_size = params.max_body_size();
