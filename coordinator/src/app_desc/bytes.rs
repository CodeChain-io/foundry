...
impl<'a> Serialize for WithoutPrefix<&'a Bytes> {
    ...
    where
        ...
        serializer.serialize_str(vec.to_hex::<String>().as_ref())
    }
}
...
mod test {
    ...
    macro_rules! assert_ok {
        ($test_str: expr, $expected: expr) => {{
            let res: Result<Bytes, _> = serde_yaml::from_str($test_str);
            assert!(res.is_ok());
    ...
    fn bytes_serialize() {
        ...
        let serialized = serde_yaml::to_string(&bytes).unwrap();
    ...
    fn bytes_serialize_without_prefix() {
        let bytes = Bytes(vec![0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef]);
        let serialized = serde_yaml::to_string(&bytes.without_prefix()).unwrap();
        assert_eq!(serialized, "---\n0123456789abcdef");
    }
