...
enum ExecuteError {
    ...
    TokenModuleError(crate::token2::Error),
    InvalidSequence,
    NotEligibleStamper,
}
...
impl ServiceHandler {
    fn excute_tx(&self, session: SessionKey, transaction: &Transaction) -> Result<(), ExecuteError> {
...
impl InitGenesis for ServiceHandler {
    fn begin_genesis(&self, _session: SessionKey) {}
    ...
    fn end_genesis(&self, _session: SessionKey) {}
}

impl TxOwner for ServiceHandler {
    fn block_opened(&self, _session: SessionKey, _: &Header) -> Result<(), HeaderError> {
        Ok(())
    }
