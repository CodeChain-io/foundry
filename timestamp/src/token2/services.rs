...
pub trait TokenManager: Service {
    // Immutable accesses
    fn get_account(&self, session: SessionKey, public: &Public, default: bool) -> Result<Account, Error>;
    fn get_owning_accounts_with_issuer(&self, session: SessionKey, issuer: &H256) -> Result<BTreeSet<Public>, Error>;

    // Mutable accesses
    fn issue_token(&self, session: SessionKey, issuer: &H256, receiver: &Public) -> Result<(), Error>;
}
...
impl TxOwner for ServiceHandler {
    fn block_opened(&self, _session: SessionKey, _header: &Header) -> Result<(), HeaderError> {
        Ok(())
    }
    ...
    fn block_closed(&self, _session: SessionKey) -> Result<Vec<Event>, CloseBlockError> {
        Ok(Vec::new())
    }
}
