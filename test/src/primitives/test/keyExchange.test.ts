import { exchange, x25519GetPublicFromPrivate } from "../src";
import "mocha";
...
describe("exchange", () => {
    ...
    it("Alice-side", () => {
        expect(exchange(BobX25519Pk, AliceX25519Sk)).equal(shared_secret);
    });

    it("Bob-side", () => {
        expect(exchange(AliceX25519Pk, BobX25519Sk)).equal(shared_secret);
    });
});

it("x25519GetPublicFromPrivate", () => {
    // The testing oracle is generated from "libsodium-wrappers"
    const x25519Sk =
        "49023b9f7f997d3c8f1763f762a2f79d3d0fc5d4204ee25bf05ef57ca190d91b";
    const x25519Pk =
    ...
    expect(x25519GetPublicFromPrivate(x25519Sk)).equal(x25519Pk);
});
