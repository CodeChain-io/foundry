...
mod tests {
    ...
    fn metadata_for_election() -> TopLevelState {
        ...
        let mut state = helpers::get_temp_state_with_metadata(params, consensus_params);
        state.metadata().unwrap().unwrap().set_consensus_params(ConsensusParams::default_for_test());
