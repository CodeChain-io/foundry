...
export class H512 {
    ...
    constructor(value: string) {
        if (!_.isString(value)) {
            throw Error(`The given value for new H512() is not a string, but ${value}`);
        }
        if ((!value.startsWith("0x") && value.length !== 128) || (value.startsWith("0x") && value.length !== 130)) {
            throw Error(`The length for H512 must be 128 or 130 with 0x-prefix, but "${value}" is given`);
